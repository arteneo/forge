...
interface FormMultiContentProps {
    ...
    onSubmitStart?: (values: FormikValues, helpers: FormikHelpers<FormikValues>) => void;
    onSubmitSuccess?: (
    ...
    onSubmitFinish?: (values: FormikValues, helpers: FormikHelpers<FormikValues>, cancelled: boolean) => void;
...
const FormMultiContent = ({
        ...
        for (const [key, requestConfig] of requestConfigs.entries()) {
            ...
            await axios
                ...
                .then((response: AxiosResponse) => {
                    onSubmitSuccess(response, values, helpers, key);
                })
    ...
    const onSubmit = async (values: FormikValues, helpers: FormikHelpers<FormikValues>) => {
        onSubmitStart(values, helpers);

        await resolveRequestConfigs(values, helpers);

        onSubmitFinish(values, helpers, cancelled);
    };
    return (
